...
def main(lang):
    classifier = EncoderClassifier.from_hparams(
        ...
        savedir="results-interim-asr-performance-gap/models",
        run_opts={"device": "cpu"},
    )
    ...
    for split in ["validation"]:  # ["train", "validation", "test"]:
        ...
        embeddings = list()
        for i, row in tqdm(
            enumerate(curr_data), desc=f"Processing {split}", leave=False, total=len(list(curr_data))
        ):
            ...
            raw_audio = torch.from_numpy(raw_audio).float().unsqueeze(0)
            embed = classifier.encode_batch(raw_audio).squeeze(0)# --> (1, hidden dim, which should be 192)
